...
#		This script has been designed and written on Unix platform (Linux, AIX, Solaris),
...
case `uname` in
    ...
    ;;
    FreeBSD ) output1=`iostat -C 5 2 | cut -b 45-56 |tr -s ' ' ';' | tail -1;`
      output2=`iostat -C 5 2 | cut -b 56- |tr -s ' ' ';' | tail -1;`
      CPU_REPORT=`echo "${output1}n/a${output2}"`
      CPU_USER=`echo $CPU_REPORT | cut -d ";" -f 2`
      CPU_SYSTEM=`echo $CPU_REPORT | cut -d ";" -f 4`
      CPU_IOWAIT=`echo $CPU_REPORT | cut -d ";" -f 5 `
      CPU_IOWAIT_INT=`echo $CPU_IOWAIT | cut -d "." -f 1 | cut -d "," -f 1 `
