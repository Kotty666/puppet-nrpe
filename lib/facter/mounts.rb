...
gem_present = if Gem::Specification.respond_to?('find_all_by_name')
                Gem::Specification.find_all_by_name('sys-filesystem').any?
              else
                Gem.available?('sys-filesystem')
              end
...
if gem_present
  ...
  if Facter.value(:kernel) == 'Linux'
    # We store hash of block devices hosting mount points here ...
    mounts = {}
    ...
    exclude = %w[afs anon_inodefs aufs autofs bdev bind binfmt_.* cgroup cifs
                 ...
                 hugetlbfs inotifyfs iso9660 lustre.* mfs mqueue ncpfs NFS nfs.* nsfs
                 none pipefs proc pstore ramfs rootfs rpc_.* securityfs shfs shm
                 smbfs sockfs sysfs systemd tmpfs udev udf unionfs usbfs]
    ...
    exclude = Regexp.union(*exclude.map { |i| Regexp.new(i) })

    # List of numeric identifiers with their corresponding canonical forms ...
    known_devices = Dir['/dev/*'].each_with_object({}) do |v, k|
      #
      ...
      # host a file system ...
      #
      next unless File.exist?(v) && File.blockdev?(v)
      ...
      #
      v = File.join('/dev', v) unless File.exist?(v)
      k.update(File.stat(v).rdev => v)

      # Yield hash back into the block ...
    end
    ...
    Facter::Util::Resolution.exec('cat /proc/mounts 2> /dev/null').each_line do |line|
      ...
      next if line.empty? || line.match(%r{^none})
      ...
      begin
        stats = Sys::Filesystem.stat(mount)
      rescue => e
        setcode { "Unable to stat mountpoint: #{e}" }
      end

      details['size'] = blocks_to_bytes(stats.block_size, stats.blocks)
      mounts[mount] = details
    end
