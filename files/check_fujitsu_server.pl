...
#----------- multi usable functions
  ...
  sub openSNMPsession 
  {
	...
	if (defined $optUserName or ($optSNMP and $optSNMP==3)) {
		...
		$snmpSessionOpts{'-version'}		= "snmpv3";
		#$snmpSessionOpts{'-community'}		= undef;
		$snmpSessionOpts{'-username'}		= $optUserName if ($optUserName);
		$snmpSessionOpts{'-authkey'}		= $optAuthKey if ($optAuthKey);
		$snmpSessionOpts{'-authpassword'}	= $optAuthPassword if ($optAuthPassword);
		$snmpSessionOpts{'-authprotocol'}	= $optAuthProt || 'md5';
		$snmpSessionOpts{'-privkey'}		= $optPrivKey if ($optPrivKey);
		$snmpSessionOpts{'-privpassword'}	= $optPrivPassword if ($optPrivPassword);
		$snmpSessionOpts{'-privprotocol'}	= $optPrivProt || 'des';
		($main::session, $error) = Net::SNMP->session(%snmpSessionOpts);
		$variableVerboseMessage .= "SNMPv3 Error=$error" 
			if ((!defined $main::session) && ($main::verbose >= 4));
	...
	if (!defined $main::session and !$optSNMP) {
		...
		$variableVerboseMessage .= "SNMP Error=$error" 
			if (!defined $main::session and $main::verbose >= 4);
	}
...
use IO::Socket;
  ...
  sub socket_checkSSM {
	...
	my $EOL = "\015\012";
	my $BLANK = $EOL x 2;
	my $document = "/ssm/desktop/index.html";
	my $remote = undef;
	...
	my $address = undef;
	if ($response and ($response =~ m/HTTP.*301/ or $response =~ m/^\s*$/)) {
	    $address = "https://$optHost:3172$document" if ($optHost !~ m/:/);
	    $address = "https://[$optHost]:3172$document" if ($optHost =~ m/:/);
	}
	return $address;
  } #socket_checkSSM
  ...
  sub socket_getSSM_URL {
	...
	if ($chkSSM) {
		$ssmAddress = socket_checkSSM($optHost);
	}
	return $ssmAddress;
  } #socket_getSSM_URL
...
sub addCelsius {
	...
	my $tmp = '';
	if ($current) {
		$tmp .= " Temperature=$current" . "C";
		$tmp .= " Warning=$warning" . "C" if (defined $warning);
		$tmp .= " Critical=$critical" . "C" if (defined $critical);
	}
	addMessage($container,$tmp);
}
sub addmVolt {
		...
		if ($getInfos) {
			...
			if (!$main::useNotify) {
				if ($verbose) {
					RFC1213sysinfoToLong();
					$longMessage .= "\n";
					addName("l",$designation);
					addLocationContact("l",$location, $contact);
...
sub primergyManagementBlade {
	...
	if ($exitCode == 1 or $exitCode == 2 or $main::verbose >= 1) {
		my $admURL = trySNMPget($snmpOidBladeAdmURL,"AgentAdminURL");
		RFC1213sysinfoToLong();
		addAdminURL("l",$admURL) if (!$main::useNotify);
		$longMessage .= "\n" if ($admURL and !$main::useNotify);
		addAdminURL("n",$admURL);
	}
	if (defined $optChkSystem) { 
		if (($main::verbose >= 1)
...
sub primergyServerFanTable {
	...
	if (($optChkEnvironment or $optChkEnv_Fan) and $getInfos) {
		...
		foreach my $fanID (@snmpIDs) {
			...
			if ($verbose) {
				addStatusTopic("v",$srvFanStatus[$fanStatus], "Fan", $fanID =~ m/\d+\.(\d+)/)
				    if (!$fanType or $fanType < 3);
				addStatusTopic("v",$srvFanStatus[$fanStatus], "Liquid", $fanID =~ m/\d+\.(\d+)/)
			...
			and   (($fanStatus == 2) || ($fanStatus == 4) || ($fanStatus == 5) ||($fanStatus == 6))
				...
				addStatusTopic("l",$srvFanStatus[$fanStatus], "Fan", $fanID =~ m/\d+\.(\d+)/);
				addName("l",$fanDesc);
				addKeyRpm("l","Speed", $fanSpeed);
				$longMessage .= "\n";
			}
...
sub primergyServerTemperatureSensorTable {
	...
	if (($optChkEnvironment or $optChkEnv_Temp) and $getinfos) {
		...
		foreach my $tempID (@snmpIDs) {	
			...
			if ($verbose) {
				addStatusTopic("v",$srvTempStatus[$tempStatus],"Sensor",$tempID=~m/\d+\.(\d+)/);
				addName("v",$tempDesc);
				addCelsius("v",$tempValue, $tempWarn,$tempCrit);
				$variableVerboseMessage .= "\n";
			} elsif ($notify
			and   ($tempStatus>=4 and $tempStatus!=8) 
				...
				addStatusTopic("l",$srvTempStatus[$tempStatus],"Sensor",$tempID=~m/\d+\.(\d+)/);
				addName("l",$tempDesc);
				addCelsius("l",$tempValue, $tempWarn,$tempCrit);
				$longMessage .= "\n";
			}
...
sub primergyServerPowerSupplyTable {
	...
	if ($optChkPower and $getInfos) {
		...
		foreach my $id (@snmpIdx) {
			...
			if ($verbose) {
				addStatusTopic("v",$statusText[$pstatus],"PSU",$id=~m/\d+\.(\d+)/);
				addName("v",$name);
				addKeyWatt("v","CurrentLoad", $load,
					undef,undef, undef,$max);
				$variableVerboseMessage .= "\n";
			} elsif ($notify and $pstatus >=4) {
				addStatusTopic("l",$statusText[$pstatus],"PSU",$id=~m/\d+\.(\d+)/);
				addName("l",$name);
...
sub primergyServerUnitTable {
	...
	{ # SC2 UnitTable 2.3 => 231
		...
		addTableHeader("v","Unit Table") if ($main::verboseTable == 231);
		foreach my $snmpID (@snmpIDs) {
			...
			if ($main::verboseTable == 231) { 
				addStatusTopic("v",undef, undef, $unitid);
			...
			if ($location or $contact or $admURL or $manageIP or $model) {
				if (!$main::useNotify) {
					if ($cxChassisName or $cxChassisModel) {
						...
						addStatusTopic("l",undef,"Multi Node System", undef);
						addSerialIDs("l",$cxChassisSerial, undef);
						addName("l",$cxChassisName);
						addLocationContact("l",$cxChassesLocation, $cxChassisContact);
						addProductModel("l",undef, $cxChassisModel);
						$longMessage .= "\n";
					}
					if ($main::verbose) {
						RFC1213sysinfoToLong();
						$longMessage .= "\n";
						addStatusTopic("v",undef, "SystemUnitInformation", undef);
						addName("v",$name);
						addLocationContact("v",$location, $contact);
						addAdminURL("v",$admURL);
						addKeyValue("v","ManagementIP",$manageIP);
						addProductModel("v",undef, $model);
						if ($main::verbose == 1) {
							chop($longMessage); # remove previous \n
							addProductModel("l",undef, $model);
							addAdminURL("l",$admURL);
							addKeyValue("l","ManagementIP",$manageIP);
							$longMessage .= "\n";
						}
						$variableVerboseMessage .= "\n";
...
sub primergyServer {
	...
	inv_sniInventoryOSinformation() if (!$cxChassisName and !$cxChassisModel);
  ...
  sub svupdComponentTable {
	...
	$fileHost =~ s/[^A-Z,a-z,.,\-,0-9]//g;
	{
		...
		foreach my $id (@snmpIdx) {
			...
			addExitCode($sevExitCode) if (defined $sevExitCode);
...
if (defined $optChkUpTime) {
	...
	if (!defined $optPrimeQuest and !defined $optBlade and !defined $optBladeContent) {
		primergyServerNotifyData();
...
$notifyMessage = undef if ($main::verbose < 9 and !$main::useNotify);
if ($longMessage) {
